...
let dbItems = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, "worldofwarcraft-icons.json"),
    "utf-8"
  )
);

let modItems = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "wow-icons.json"), "utf-8")
);
...
for (let modItem of modItems) {
  if (modItem.type === undefined) {
    ...
    console.log(
      `${modItem.name} => ${filtered.map(item => item.type).join(", ")}`
    );
  } else {
    console.log(
      `Item ${modItem.name} has type ${modItem.type}, let's check for subtype`
    );
